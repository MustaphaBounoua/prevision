...
def test_arimax(datah,data,testdate,nbjourpred,nbjourtest,p=2,d=0,q=2,P=0,D=1,Q=2,s=7):
    from sklearn.metrics import mean_squared_error
    from math import sqrt
    horizon = nbjourpred
    nbjourtest = nbjourtest
...
##### horizon une journée
    end_test = test_date_time + timedelta(days=1)
    test = datah[test_date_time:end_test]
    test.drop(test.tail(1).index, inplace=True)
    prevision = arima_model.predict(start=train.shape[0], end=train.shape[0] - 1 + 1 )
    res=[]
    for i in range(1):
        for m in daily:
            res.append(prevision[i]*m/100)

    mae = mean_absolute_error(test.nb, res)
    rmse = sqrt(mean_squared_error(test.nb, res))
    result["MAE1"] = mae
    result["rmse1"] = rmse
#### horizon 2 journée
    ...
    test = datah[test_date_time:end_test]
    test.drop(test.tail(1).index, inplace=True)
    prevision = arima_model.predict(start=train.shape[0], end=train.shape[0] - 1 + 2)
    res = []
    for i in range(2):
        for m in daily:
            res.append(prevision[i] * m / 100)
    ...
    test = datah[test_date_time:end_test]
    test.drop(test.tail(1).index, inplace=True)
    prevision = arima_model.predict(start=train.shape[0], end=train.shape[0] - 1 + 3)
    res = []
    for i in range(3):
        for m in daily:
            res.append(prevision[i] * m / 100)
    ...
    test = datah[test_date_time:end_test]
    test.drop(test.tail(1).index, inplace=True)
    prevision = arima_model.predict(start=train.shape[0], end=train.shape[0] - 1 + 4)
    res = []
    for i in range(4):
        for m in daily:
            res.append(prevision[i] * m / 100)
    ...
    test = datah[test_date_time:end_test]
    test.drop(test.tail(1).index, inplace=True)
    prevision = arima_model.predict(start=train.shape[0], end=train.shape[0] - 1 + 7)
    res = []
    for i in range(7):
        for m in daily:
            res.append(prevision[i] * m / 100)
...
def cross_validation_arimax(a,data,start_date,horizon,nbjour_max,step,p=3,d=0,q=3,P=0,D=1,Q=2,s=24,train_size=[365]):
    from datetime import timedelta
    from pandas import datetime
    result = []
    inter = []
    ...
    for m in train_size:
        while k < nbjour_max:
            ...
            print(str(date))
            try:
                precision = test_arimax(a,data, date.strftime('%d/%m/%Y'), horizon, m ,2 , 0 , 2 , 0 , 1 , 2, 7)
